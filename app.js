/* ===========================================================
   FIBEL — „Litera D cu Herzchen" · Claudia Toth © 2026
   Abecedar german interactiv pentru copii 4–8 ani.
   Temă: Drachen 🪁 · „D wie Drachen" · „Herzchen lässt den Drachen steigen".

   17 secțiuni: Start · Litera D · Suche D · Cuvinte · Citește (cuvinte+propoziții) ·
   Legare silabe (unește) · Despărțire în silabe (clap) · Joc D · Wo hörst du D ·
...
const D = {
  drachen: {de:'der Drachen', icon:'drachen', audio:'d-drachen', audioArt:'d-art-drachen', gen:'m', art:'der', ro:'zmeul',    m:true},
  delfin:  {de:'der Delfin',  icon:'delfin',  audio:'d-delfin', audioArt:'d-art-delfin',  gen:'m', art:'der', ro:'delfinul', m:true},
  dose:    {de:'die Dose',    icon:'dose',    audio:'d-dose', audioArt:'d-art-dose',    gen:'f', art:'die', ro:'conserva', m:true},
  decke:   {de:'die Decke',   icon:'decke',   audio:'d-decke', audioArt:'d-art-decke',   gen:'f', art:'die', ro:'pătura',   m:true},
  dach:    {de:'das Dach',    icon:'dach',    audio:'d-dach', audioArt:'d-art-dach',    gen:'n', art:'das', ro:'acoperișul',m:true},
  daumen:  {de:'der Daumen',  icon:'daumen',  audio:'d-daumen', audioArt:'d-art-daumen',  gen:'m', art:'der', ro:'degetul mare',m:true},
  maus:  {de:'die Maus',  icon:'m-maus',audio:'d-maus',  gen:'f', art:'die', ro:'șoricelul', m:false},
  mond:  {de:'der Mond',  icon:'m-mond',audio:'d-mond',  gen:'m', art:'der', ro:'luna',      m:false},
  katze: {de:'die Katze', icon:'katze', audio:'d-katze', gen:'f', art:'die', ro:'pisica',    m:false},
  apfel: {de:'der Apfel', icon:'apfel', audio:'d-apfel', gen:'m', art:'der', ro:'mărul',     m:false},
  obst:  {de:'das Obst',  icon:'obst',  audio:'d-obst',  gen:'n', art:'das', ro:'fructele',  m:false},
  tiger: {de:'der Tiger', icon:'tiger', audio:'d-tiger', gen:'m', art:'der', ro:'tigrul',    m:false},
};
const WORDS_D  = ['drachen','delfin','dose','decke','dach','daumen'];
const GAME_POOL= ['drachen','maus','delfin','katze','dose','apfel'];
const ART_QUIZ = ['drachen','dose','dach','delfin'];
const GRID = [
  ['D','m','d','N','w'],['l','d','M','D','n'],['D','o','d','b','D'],['t','D','a','d','F'],['b','d','M','D','u'],
];
/* despărțire în silabe (clap + audio IPA pe silabă) — 1 → 2 → 3 silabe */
const SPLIT = [
  {icon:'dach',   audio:'d-dach',   syl:['Dach'],         sylA:['d-syl-dach']},
  {icon:'delfin', audio:'d-delfin', syl:['Del','fin'],    sylA:['d-syl-del','d-syl-fin']},
  {icon:'banane', audio:'d-banane', syl:['Ba','na','ne'], sylA:['d-syl-ba','d-syl-na','d-syl-ne']},
];
const STORY = [
  {de:'Das ist <b class="m-bold">Herzchen</b>.',                                       ro:'Acesta e Herzchen (inimioara).', icon:'herzchen', audio:'st-1'},
  {de:'<b class="m-bold">Herzchen</b> sieht einen <b class="m-bold">Drachen</b>.',      ro:'Herzchen vede un zmeu.',         icon:'drachen',  audio:'st-2'},
  {de:'<b class="m-bold">Herzchen</b> lässt den <b class="m-bold">Drachen</b> steigen.',ro:'Herzchen înalță zmeul.',         icon:'drachen',  audio:'st-3'},
  {de:'<b class="m-bold">Herzchen</b> ist glücklich.',                                 ro:'Herzchen e fericit.',            icon:'herzchen', audio:'st-4'},
];
const QUIZ = [
  {type:'audio',   ans:'drachen', opts:['drachen','maus','katze']},
  {type:'findm',   ans:'delfin',  opts:['delfin','katze','mond']},
  {type:'article', word:'dach'},
  {type:'audio',   ans:'dose',    opts:['dose','apfel','tiger']},
  {type:'findm',   ans:'daumen',  opts:['daumen','obst','mond']},
];
const LETTER = 'D';
const POS = [
  {icon:'drachen', audio:'d-drachen', pos:'a'},
  {icon:'delfin',  audio:'d-delfin',  pos:'a'},
  {icon:'dose',    audio:'d-dose',    pos:'a'},
  {icon:'dach',    audio:'d-dach',    pos:'a'},
];
...
SCREENS[0] = function(){
  ...
  show(
    ...
    '<h1 class="title">Litera D cu Herzchen</h1>' +
  ...
  document.getElementById('startBtn').onclick = ()=>{ play('d-wie-drachen'); go(1); };
...
SCREENS[1] = function(){
  show(
    ...
    '<div class="letter-hero">D<span class="lo">d</span></div>' +
    '<div class="btn-row">' +
      '<button class="big-btn" id="bName">🔊 D</button>' +
      '<button class="big-btn amber" id="bMaus">🔊 D wie Drachen</button>' +
    '</div>' +
    '<p class="parent-note">Atinge: „D" — litera se citește „de" · „D wie Drachen" = D ca în Drachen (zmeu).</p>' +
    nextBtn()
  );
  document.getElementById('bName').onclick = ()=>play('d-name');
  document.getElementById('bMaus').onclick = ()=>play('d-wie-drachen');
  setTimeout(()=>play('d-wie-drachen'), 350);
  wireNext(2);
};
...
SCREENS[2] = function(){
  ...
  GRID.forEach(row=> row.forEach(ch=>{
    const isM = (ch==='D'||ch==='d'); if(isM) total++;
    cells += '<div class="lcell" data-m="'+(isM?1:0)+'">'+ch+'</div>';
  }));
  show(
    '<p class="kicker">Suche D und d 🔍</p>' +
    '<h1 class="title" style="font-size:1.4rem">Atinge toți D și d</h1>' +
...
SCREENS[3] = function(){
  ...
  show(
    ...
    '<h1 class="title" style="font-size:1.5rem">Cuvinte cu D</h1>' +
    '<div class="cards">'+cards+'</div>' + nextBtn()
  );
  stage.querySelectorAll('.card').forEach(el=>{ el.onclick = ()=>{ el.classList.add('tapped'); play(D[el.dataset.k].audioArt||D[el.dataset.k].audio); }; });
  wireNext(4);
};
...
SCREENS[4] = function(){
  ...
  const SENT = [
    {t:'Dora lernt.', a:'st-doralernt'}, {t:'Udo lernt.', a:'st-udolernt'}
  ];
  ...
  show(
    ...
    '<h1 class="title" style="font-size:1.1rem">Cu literele învățate + D</h1>' +
...
SCREENS[5] = function(){
  const LEFT  = ['Do','No','Mo','O'];
  const RIGHT = ['se','ra','na','ma'];
  const VALID = {
    'Do|se': {w:'Dose', a:'d-dose'},
    'No|ra': {w:'Nora', a:'d-nora'},
    'Mo|na': {w:'Mona', a:'d-mona'},
    'O|ma':  {w:'Oma',  a:'d-oma'}
  };
...
/* 7 — JOC: beginnt mit D? */
SCREENS[7] = function(){
  let tiles = spreadShuffle(GAME_POOL, k=>D[k].m, 2).map(k=>
    '<div class="tile" data-k="'+k+'" style="position:relative"><img src="'+ICON+D[k].icon+'.svg" alt="'+D[k].de+'"></div>'
  ).join('');
  show(
    hero('small') +
    '<p class="game-q">Atinge ce începe cu <span class="m-hi">D</span>!</p>' +
    '<div class="game-grid">'+tiles+'</div>' +
    '<p class="parent-note">Welches Bild beginnt mit D?</p>'
  );
  let found = 0; const need = GAME_POOL.filter(k=>D[k].m).length;
  stage.querySelectorAll('.tile').forEach(el=>{
    el.onclick = function(){
      const w = D[el.dataset.k];
  ...
  function card(){
    ...
    show(
      ...
      '<div class="flashnav">' +
        ...
        '<span class="counter">'+(i+1)+' / '+WORDS_D.length+'</span>' +
    ...
    pv.disabled = (i<=0); nx.disabled = (i>=WORDS_D.length-1);
    pv.onclick = ()=>{ if(i>0){ i--; card(); } };
    nx.onclick = ()=>{ if(i<WORDS_D.length-1){ i++; card(); } };
    wireNext(12);
  }
  card();
};

/* 12 — TRASEAZA D / d — cu SĂGEȚI */
SCREENS[12] = function(){
  let glyph = 'D';
  const STROKE = {'D':'stroke-big','d':'stroke-small'};
  function render(){
    show(
      '<p class="kicker">Schreiben · ① urmează săgețile verzi ✏️</p>' +
      '<div class="btn-row" style="margin-bottom:2px">' +
        '<button class="big-btn'+(glyph==='D'?'':' amber')+'" id="gM">D mare</button>' +
        '<button class="big-btn'+(glyph==='d'?'':' amber')+'" id="gm">d mic</button></div>' +
    ...
    document.getElementById('gM').onclick=()=>{glyph='D';render();};
    document.getElementById('gm').onclick=()=>{glyph='d';render();};
...
/* 14 — SCRIE PROPOZIȚIA */
SCREENS[14] = function(){
  const GLYPH = {'N':'s-cap-n','S':'s-cap-s','R':'s-cap-r','L':'s-cap-l','M':'s-cap-m','T':'s-cap-t','O':'s-cap-o','E':'s-cap-e','F':'s-cap-f','U':'s-cap-u','B':'s-cap-b','D':'s-cap-d',
                 'n':'s-n','s':'s-s','r':'s-r','t':'s-t','m':'s-m','a':'s-a','i':'s-i','l':'s-l','o':'s-o','e':'s-e','u':'s-u','b':'s-b','d':'s-d'};
  const SENT = [
    {t:'Dora lernt.', a:'st-doralernt'},
    {t:'Udo lernt.', a:'st-udolernt'}
  ];
  ...
  function q(){
    ...
    if (it.type==='audio'){
      body = '<p class="game-q">Ascultă 🔊 și atinge imaginea</p>' +
        ...
        '<div class="opt-grid">'+ shuffle(it.opts).map(k=>'<div class="opt" data-k="'+k+'"><img src="'+ICON+D[k].icon+'.svg" alt=""></div>').join('') +'</div>';
    } else if (it.type==='findm'){
      body = '<p class="game-q">Care începe cu <span class="m-hi">D</span>?</p>' +
        '<div class="opt-grid">'+ shuffle(it.opts).map(k=>'<div class="opt" data-k="'+k+'"><img src="'+ICON+D[k].icon+'.svg" alt=""></div>').join('') +'</div>';
    } else {
      const w = D[it.word];
    ...
    if (it.type==='audio'){
      ...
      stage.querySelectorAll('.opt').forEach(el=> el.onclick = function(){
        if (el.dataset.k === it.ans){ el.classList.add('good'); play(D[el.dataset.k].audio); win(); } else wrong(el);
      });
    } else if (it.type==='findm'){
      stage.querySelectorAll('.opt').forEach(el=> el.onclick = function(){
        if (D[el.dataset.k].m){ el.classList.add('good'); play(D[el.dataset.k].audio); win(); } else wrong(el);
      });
    } else {
      const w = D[it.word]; play(w.audio);
...
SCREENS[16] = function(){
  show(
    ...
    '<div class="story" style="max-width:380px"><div class="story-row read" id="rima">' +
      '<img src="'+ICON+'drachen.svg" alt="">' +
      '<div><div class="s-de"><b class="m-bold">D</b> wie <b class="m-bold">Drachen</b>.<br><b class="m-bold">Herzchen</b> lässt den <b class="m-bold">Drachen</b> steigen.</div>' +
      '<div class="s-ro">D ca în Drachen. Herzchen înalță zmeul.</div></div>' +
...
function printDiploma(){
  ...
  document.getElementById('diploma-print').innerHTML =
    '<div class="dip">' +
      ...
      '<div class="dip-letter">D d</div>' +
      '<div class="dip-text">D wie Drachen — mit Herzchen! 🧡</div>' +
...
function printColoring(){
  ...
  document.getElementById('coloring-print').innerHTML =
    '<div class="color-page">' +
      ...
      '<h2 class="cp-title">D — Herzchen lässt den Drachen steigen</h2>' +
      '<img src="'+ICON+'herzchen-color.svg" alt="Herzchen lässt den Drachen steigen">' +
      '<div class="cp-sentence">Herzchen lässt den Drachen steigen.</div>' +
...
(function initParent(){
  ...
  if (wl) wl.innerHTML = WORDS_D.map(k=>'<span>'+D[k].de+'</span>').join('');
